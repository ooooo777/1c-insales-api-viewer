...
и понять, где искать ошибку уже работающего обмена. Обработка не создаёт и не меняет в базе документы,
справочники и регистры; свои настройки она хранит в хранилище общих настроек. Один файл .epf,
от конфигурации не зависит, открытый код под MIT.
...
Проверено на платформе 8.5.1.1343 в файловом и в клиент-серверном режиме. Не проверялось:
веб-клиент, 1С:Фреш, минимальный релиз платформы.

Нужен ключ доступа к API магазина (идентификатор и пароль). Ключ создаётся в кабинете магазина,
в разделе для разработчиков, пункт «Ключи доступа».

Обработку можно открыть через «Файл», «Открыть»: для этого нужно право интерактивного открытия
внешних обработок, в типовых базах оно обычно есть только у администратора. Если в базе есть
механизм дополнительных обработок, её можно подключить через «Дополнительные отчёты и обработки».
